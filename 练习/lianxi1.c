...
int main(){
    ...
    if(flag==0){    //第一、二种情况
        ...
        printf("情况3\n");
#endif
        char* s_sub=(char *)malloc(sizeof(char)*Size);//子串，含x,y变量的
        i=0;
        while(i<length&&s[i]!='|')
            i++;
		//截取表达式 
        ...
        while(i<length){
            //获取变量
            char* st=(char *)malloc(sizeof(char)*Size); //临时变量,记录每个变量
            int st_i=0;//记录st的下标
            while(i<length&&s[i]!=':'){
//                st+=s[i];
                st[st_i]=s[i];
        ...
        for(k=vars[0].start;k<vars[0].end;k+=vars[0].bc){
            for(l=vars[1].start;l<vars[1].end;l+=vars[1].bc){
                ...
                char* pos;
                pos = strstr(ss,vars[0].var_name); //查找指定的串,返回第一次出现的位置
                ...
                pos = strstr(ss,vars[1].var_name);//查找指定的串
                while (pos != NULL)
                {
                    char num_str[10];
                    sprintf(num_str,"%d",l);
                    replace(ss,pos,strlen(vars[1].var_name),num_str);//用新的串替换掉指定的串
                    pos = strstr(ss,vars[1].var_name);//继续查找指定的串，直到所有的都找到为止
                }
#ifdef debug
                //输出替换后的字符串
                printf("ss:%s\n",ss);
